<h1 align="center">Hi 👋, I'm Anay Pradhan</h1>

<h3 align="center">
Building AI-powered systems for behavior change, health, and real-world impact
</h3>

---

### 👨‍💻 About Me

- 🎓 Senior Computer Science student at Arizona State University  
- 🧠 Passionate about building **AI systems that drive real behavior change**, not just generate responses  
- 🏋️ PN-L1 Certified Nutrition Coach with a background in **behavior change psychology**  
- 🚀 Currently focused on combining **AI + health + psychology** into impactful applications  

---

### 🚀 Currently Building

- 🧠 **AI Nutrition Coach (Behavior-Driven AI System)**  
  Designing an AI system that goes beyond generic advice by using structured prompts and curated knowledge to drive real behavior change  

- 📊 Building prompt frameworks + knowledge bases for **personalized coaching experiences**  

- 🔁 Applying principles of **habit formation, adherence, and motivation psychology**  

---

### 💡 What I Care About

I’m interested in building systems that don’t just give information, but actually help people change their behavior.

My focus lies at the intersection of:
- AI systems (LLMs, prompt design)
- Behavioral psychology
- Real-world user impact (health, habits, learning)

---

### 🛠 Skills

**Languages:** Python, Java, JavaScript, C++  
**Frameworks:** React, Node.js, Flask, JavaFX  
**Databases:** SQL, MongoDB  
**Tools:** Git, GitHub, Postman, VS Code  
**Core Areas:** REST APIs, OpenAI API, Backend Development  

---

### 🛠 Tech Stack

**Languages:**  
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java&logoColor=white)
![Python](https://img.shields.io/badge/Python-14354C?style=flat&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)

**Frameworks & Libraries:**  
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![JavaFX](https://img.shields.io/badge/JavaFX-1D2951?style=flat&logo=java&logoColor=white)

**Developer Tools:**  
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)

**Database & Hosting:**  
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

---

### 📌 Projects

#### 🧠 AI Nutrition Coach (In Progress)  
[GitHub](YOUR_REPO_LINK_HERE)

- Building an AI-powered coaching system focused on **real behavior change**, not generic responses  
- Designing structured prompt systems and curated knowledge base for personalized guidance  
- Integrating principles from **nutrition coaching (PN-L1)** and **behavior psychology**  
- Actively developing with continuous improvements and feature iterations  

---

#### 🧠 SERA – AI Sexual Health Chatbot  
[Frontend](https://github.com/rahulbaweja7/AI-SexDoc-FrontEnd.git) | [Backend](https://github.com/rahulbaweja7/Ai-SexDoc-BackEnd.git)

- Built an AI-powered chatbot using OpenAI API with a secure backend and responsive frontend  
- Designed structured prompt flows for safe, contextual, and user-aware responses  
- Collaborated in Agile development cycles including sprint planning and iterative testing  
- 🏆 **2nd Place – WiCS Hacks 2025**

---

### 📫 Connect with Me

- 📧 **Email:** anaypradhan3849@gmail.com  
- 💼 **LinkedIn:** [linkedin.com/in/anay-pradhan](https://linkedin.com/in/anay-pradhan)  

---
